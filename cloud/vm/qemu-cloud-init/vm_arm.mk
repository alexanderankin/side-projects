build/$(UBUNTU_CODENAME)-server-cloudimg-armhf.img_: build/.check_wget
	$(shell cd build; wget -N $(IMG_BASE_URL)-armhf.img)
	touch $@

# this must happen every time if you want a clean vm
.PHONY: build/vm_arm.img
build/vm_arm.img: build/$(UBUNTU_CODENAME)-server-cloudimg-armhf.img_
	qemu-img create -f qcow2 -b $(UBUNTU_CODENAME)-server-cloudimg-armhf.img -F qcow2 build/vm_arm.img 20G
